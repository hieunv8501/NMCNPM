...
namespace QLDKMH_CNPM.Areas.PDT.Controllers
{
    public class DS_MONHOC_MOController : Controller
    {
        ...
        public ActionResult Upload(FormCollection formCollection)
        {
            ...
            return RedirectToAction("Index", "DS_MONHOC_MO", new { code = 10});
        }
        ...
        public ActionResult Index(int id, int code = 0)
        {
            ViewBag.code = code;
        ...
        public ActionResult Create(int MaHKNH, string MaMonHoc)
        {
            DS_MONHOC_MO dS_MONHOC_MO = new DS_MONHOC_MO();
            dS_MONHOC_MO.MaHKNH = MaHKNH;
            dS_MONHOC_MO.MaMonHoc = MaMonHoc;
            int count = db.DS_MONHOC_MO.Where(x => (x.MaMonHoc == MaMonHoc)).Count();
            string s = "_L";
            if (count == 0)
            {
                count = 1;
                dS_MONHOC_MO.MaMo = MaMonHoc + s + "0" + count.ToString();
            }
            else
            {
                //Neu nho hon 10 thi them so o dang truoc L
                if (count + 1 < 10)
                {
                    dS_MONHOC_MO.MaMo = MaMonHoc + s + "0" + (count + 1).ToString();
                }
                else
                { dS_MONHOC_MO.MaMo = MaMonHoc + s + (count + 1).ToString(); }
            }
            try
            {
            ...
            {
                return RedirectToAction("Create", "DS_MONHOC_MO", new { code = 1 });
            }
