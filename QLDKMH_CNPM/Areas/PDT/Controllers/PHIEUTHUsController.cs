    ...
    public class PHIEUTHUsController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "SoPhieuThu,SoPhieuDKHP,NgayLap,SoTienThu")] PHIEUTHU pHIEUTHU)
        {
            try
            {
                ...
                ViewBag.SoPhieuDKHP = new SelectList(db.PHIEU_DKHP.Where(p => p.SoTienConLai > 0 && DateTime.Now <= p.HKNH.HanDongHocPhi), "SoPhieuDKHP", "SoPhieuDKHP");
                return View(pHIEUTHU);
            }
            catch (Exception)
            {
        ...
        public ActionResult Edit(int? id, int code = 0)
        {
            ...
            ViewBag.SoPhieuDKHP = new SelectList(db.PHIEU_DKHP.Where(p => p.SoTienConLai > 0), "SoPhieuDKHP", "SoPhieuDKHP", pHIEUTHU.SoPhieuDKHP);
            return View(pHIEUTHU);
        }
