...
namespace QLDKMH_CNPM.Areas.PDT.Controllers
{
    public class DSSV_CHUAHOANTHANH_HPController : Controller
    {
        ...
        public JsonResult GetID(string temp1, string temp2)
        {
            int i = Convert.ToInt32(temp1);
            int j = Convert.ToInt32(temp2);
            IList<DS_model> DS = new List<DS_model>();
            var model = from a in db.PHIEU_DKHP
                        join b in db.SINHVIENs on a.MaSV equals b.MaSV
                        join c in db.HKNHs on a.MaHKNH equals c.MaHKNH
                        where ((a.SoTienConLai > 0) && (c.HocKy == i) && (c.Nam1 == j))
                        select new DS_model()
                        {
                            MaHKNH = a.MaHKNH,
                            MaSV = b.MaSV,
                            ...
                            Nam1 = c.Nam1,
                            Nam2 = c.Nam2,
                            SoTienConLai = (decimal)(from a in db.PHIEU_DKHP select a.SoTienConLai).Sum()
                        };
            DS = model.ToList();
